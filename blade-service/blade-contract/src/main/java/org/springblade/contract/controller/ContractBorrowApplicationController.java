...
public class ContractBorrowApplicationController extends BladeController {
	...
	private final IContractFormInfoService formInfoService;
	...
	public R save(@Valid @RequestBody ContractBorrowApplicationResponseVO contractBorrowApplication) {
		ContractBorrowApplicationEntity entity = new ContractBorrowApplicationEntity();
		BeanUtil.copy(contractBorrowApplication, entity);
		entity.setBorrowStatus(BORROW_APPLICATION_STATUS);
		R<User> user = userClient.userInfoById(AuthUtil.getUserId());
		entity.setApplicantCode(String.valueOf(user.getData().getCode()));
		log.info("借阅申请信息：" + entity);
		R<EkpVo> ekpVo = abutmentClient.borEkpFormPost(entity);
