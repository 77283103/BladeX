...
public class ContractPerformanceColPayController extends BladeController {
	...
	/**
	 * 导出excel
	 * @param performanceColPayEntity
	 * @param response
	 */
	@PostMapping("/exportTargetDataResult")
	@ApiOperationSupport(order = 7)
	@ApiOperation(value = "导出", notes = "")
	public void exportTargetDataResult(@RequestBody ContractPerformanceColPayResponseVO performanceColPayEntity, HttpServletResponse response) {

		if (Func.isNotEmpty(performanceColPayEntity)) {
			/* 导出文件名称 */
			...
			/*合同名称*/
			cloumns.add(performanceColPayEntity.getContractFormInfoEntity().getContractName());
			/*合同编号*/
			cloumns.add(performanceColPayEntity.getContractFormInfoEntity().getContractNumber());
			/*相对方名称*/
			StringBuilder name = new StringBuilder();
			for (ContractCounterpartEntity counterpartEntity : performanceColPayEntity.getCounterpartEntityList()) {
			...
			/*服务内容*/
			cloumns.add(performanceColPayEntity.getName());
			/*交易类型*/
			cloumns.add(bizClient.getValue("transaction_type",performanceColPayEntity.getType()).getData());
			/*计划缴纳时间*/
			cloumns.add(performanceColPayEntity.getPlanPayTime());
			/*计划缴纳金额*/
			cloumns.add(performanceColPayEntity.getPlanPayAmount());
			/*实际缴纳时间*/
			cloumns.add(performanceColPayEntity.getActualPayTime());
			/*实际缴纳金额*/
			cloumns.add(performanceColPayEntity.getActualPayAmount());
			/*计划退回时间*/
			cloumns.add(performanceColPayEntity.getPlanReturnTime());
			/*计划缴纳金额*/
			cloumns.add(performanceColPayEntity.getPlanReturnAmount());
			/*实际退回时间*/
			cloumns.add(performanceColPayEntity.getActualReturnTime());
			/*实际退回金额*/
			cloumns.add(performanceColPayEntity.getActualReturnAmount());
			/*厂别*/
			cloumns.add(performanceColPayEntity.getFactories());
			/*账期*/
			cloumns.add(performanceColPayEntity.getAccountPeriod());
			/*接受条件*/
			cloumns.add(performanceColPayEntity.getAcceptanceConditions());
			/*状态*/
			cloumns.add(performanceColPayEntity.getStatus());
			data.add(cloumns);
